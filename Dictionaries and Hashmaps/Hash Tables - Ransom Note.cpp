...
{
    //Create hashmap to store string and number of instances string appears
    unordered_map<string, int> Map;
    int magSize = magazine.size();
    int noteSize = note.size();

    //Fill map with strings from magazine
    for(int i = 0; i < magSize; i++)
    {
      Map[magazine[i]] = Map[magazine[i]] + 1;
    }

    //Compare note strings to strings in map, decrement if string is present
    for (int i = 0; i < noteSize; i++)
    {
        if(Map[note[i]] > 0)
        {
          Map[note[i]] = Map[note[i]] - 1;
        }
        else
        {
            cout << "No" << endl;
